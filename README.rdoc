...
* Note: To create embeded image, you must run Astah.export_diagrams() periodically by some scheduler such as cron.
...
* Embed image which is exported from the diagram which is described by astah*.
    ...
    Note: Diagram path follows to astah*'s export function.
          If your diagram name has '/', the function convert it to '_'.
...
1. Copy the plugin tree into #{RAILS_ROOT}/vendor/plugins/
    ...
    #{RAILS_ROOT}/vendor/
      plugins/
        redmine_wiki_astah/
2. Configure variavle value in redmine_wiki_astah/run-astah.sh
...
5. To create embeded image, you must configure to execute Astah.export_diagrams() periodically.
